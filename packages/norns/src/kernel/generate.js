...
function hasPages(specs) {
	return Object.values(specs.modules).some((m) => Object.keys(m?.pages ?? {}).length > 0);
}

/** The blank-state `/` of an app with no Pages yet (D87). */
export function blankPageFile(specs) {
	const name = specs.app?.name ?? 'norns app';
	return {
		path: 'routes/+page.svelte',
		text: [
			'<!-- GENERATED by `norns generate` — do not edit. Replaced by the first spec Page. -->',
			'<section class="norns-blank">',
			`\t<h1>${escapeHtml(name)}</h1>`,
			'\t<p>No pages yet. Declare a module with a <code>Page</code> in <code>specs/</code> — this screen disappears when the first one exists.</p>',
			'</section>',
			''
		].join('\n')
	};
...
export function generateApp(dir, opts = {}) {
	...
	// D87: a page-less app (the clean starter) still answers `/` with a
	// blank-state screen; it is pruned the moment any spec Page exists.
	if (!hasPages(specs)) {
		appFiles.push('routes/+page.svelte');
		if (pending.length > 0 || !existsSync(join(outRoot, 'routes', '+page.svelte'))) {
			pending.push(blankPageFile(specs));
		}
	}
